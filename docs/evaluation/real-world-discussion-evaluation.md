...
### Live casual-conversation continuity and sparse-map follow-up

A later production practice session ended after 44 Provider Finals because a known, automatic Server VAD item of about 1.1 seconds completed with an empty transcript and no deltas. This was **not** an empty explicit commit. The item contained local PCM energy, but available metadata cannot establish whether it held speech, BGM, or noise. The session's Graph had six non-Topic Nodes and four Topic-containment edges, but no accepted semantic Node-to-Node edge. Focused Flow therefore showed only its latest focal Node; this was a sparse-Projection behavior, not Graph data loss.

The follow-up changes the default for **known, at-most-3-second, no-delta, automatic VAD empty items only** to continue capture while counting a possible Evidence gap and persistently warning the facilitator. Unknown ranges, longer turns, partial recognition, and explicit commits remain fatal. An `ended` state with a nonzero possible-gap count does **not** mean Evidence loss zero. The Focused Flow separately shows up to two recently recorded discussion points when the focal point has no accepted semantic edge, without drawing invented lines or changing the Canonical Graph. The user-facing heading is simply 「最近出た話」.

Candidate `casual-session-rc1` was published from merge commit `609897f` through GitHub Actions run `35999790622` and deployed by digest `sha256:8c0be036b3e6385b752852db0cf3ba25d68e0a3372c1c383534dd90c845c0077`. Local suite: 282 tests, zero failures. Deployment: Pod Ready 1/1, restart 0, health/ready and participant/control routes 200. A synthetic production smoke with speech → 35 seconds of low-level background tone → speech remained active without explicit commit and drained to `ended`, queue 0/0/0, Graph/Render 2/2. This smoke did **not** reproduce an empty Provider Final; continuation after a short empty VAD item is covered by item-scoped local tests, not yet real-provider observation. Human casual-conversation acceptance, possible-gap safety review, and semantic Relation quality remain open. This deployment does not authorize Pilot.
